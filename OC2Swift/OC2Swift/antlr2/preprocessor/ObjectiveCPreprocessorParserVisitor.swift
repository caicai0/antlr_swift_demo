...
open class ObjectiveCPreprocessorParserVisitor<T>: ParseTreeVisitor<T> {
	...
	/**
	 * Visit a parse tree produced by the {@code preprocessorCode}
	 * labeled alternative in {@link ObjectiveCPreprocessorParser#text}.
	- Parameters:
	  - ctx: the parse tree
	- returns: the visitor result
	 */
	open func visitPreprocessorCode(_ ctx: ObjectiveCPreprocessorParser.PreprocessorCodeContext) -> T {
	 	fatalError(#function + " must be overridden")
	}

	/**
	 * Visit a parse tree produced by the {@code preprocessorDirective}
	 * labeled alternative in {@link ObjectiveCPreprocessorParser#text}.
	- Parameters:
	  - ctx: the parse tree
	- returns: the visitor result
	 */
	open func visitPreprocessorDirective(_ ctx: ObjectiveCPreprocessorParser.PreprocessorDirectiveContext) -> T {
	 	fatalError(#function + " must be overridden")
	}
